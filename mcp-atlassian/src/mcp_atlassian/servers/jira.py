...
async defjira_create_issue(
    ...
    if jira.config.read_only:
        logger.warning("Attempted to calljira_create_issue in read-only mode.")
...
@convert_empty_defaults_to_none
@jira_mcp.tool(tags={"jira", "write"})
async defjira_create_issue_link(
    ctx: Context,
    link_type: Annotated[
        str,
        Field(
    ...
    jira = await get_jira_fetcher(ctx)
    if jira.config.read_only:
        logger.warning("Attempted to calljira_create_issue_link in read-only mode.")
